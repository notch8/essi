...
RSpec.describe AdminSet, type: :model do
  ...
  describe '#metadata_context' do
    it 'leverages M3::Context.find_metadata_context_for' do
      admin_set = build(:admin_set, id: 1234)
      expect(M3::Context).to receive(:find_metadata_context_for).with(admin_set_id: admin_set.id).and_return(:context)
