...
RSpec.describe M3::Validator do
  ...
  describe 'invalid schema' do
    context "when data type is invalid" do
      ...
      before do
        allow(described_class).to receive(:validate).and_raise(M3::Validator::InvalidDataError)
      end

      it "logs error message" do
        expect { described_class.validate(data: data, schema: default_schema, logger: default_logger) }.to raise_error(M3::Validator::InvalidDataError)
      end
